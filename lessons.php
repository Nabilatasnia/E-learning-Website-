...
echo '<td><form action="editCourse.php" method="POST" class="d-inline"><input type="hidden" name="id" value='.$row['courseID'].'><button type="submit" class="btn btn-info mr-3" name="view" value="View"><i class="fa fa-edit"></i></form><form method="POST" class="d-inline"><input type="hidden" name="id" value='.$row['courseID'].'><button type="submit" class="btn btn-secondary" name="delete" value="Delete"><i class="fa fa-trash"></i></form></td>
</tr>';
 } ?>
</table><?php } else {
	echo "0 Result";
...
if (isset($_REQUEST['delete']))
	{
		$delcourseid=$_POST["id"];
		echo $delcourseid;
		$sql="DELETE FROM courses WHERE courseID={$_REQUEST['id']}";
